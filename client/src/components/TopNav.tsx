...
// ─── Team Panel ───────────────────────────────────────────────────────────────
const MOCK_MEMBERS = [
  { id: 1, name: "小A",    role: "设计师",    status: "online", avatar: "A" },
  { id: 2, name: "云团龙",  role: "产品经理",  status: "online", avatar: "Y" },
  { id: 3, name: "像素猫",  role: "前端工程师", status: "away",   avatar: "X" },
  { id: 4, name: "Jinhui", role: "设计负责人", status: "online", avatar: "J" },
];

function TeamPanel({ onClose }: { onClose: () => void }) {
  const [copied, setCopied] = useState(false);
  const inviteLink = `${window.location.origin}?invite=team`;
  ...
  const statusColor = (s: string) =>
    s === "online" ? "bg-emerald-400" : s === "away" ? "bg-amber-400" : "bg-gray-300";

  return (
    <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-2xl border border-gray-100 shadow-xl shadow-gray-200/60 z-50 overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
        <div className="flex items-center gap-2">
          <span className="text-sm font-semibold text-gray-800">团队成员</span>
          <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-50 text-emerald-600 font-medium">
            {MOCK_MEMBERS.filter(m => m.status === "online").length} 在线
          </span>
        </div>
        <button onClick={onClose} className="w-6 h-6 rounded-lg hover:bg-gray-100 flex items-center justify-center transition-colors">
        ...
        {MOCK_MEMBERS.map(m => (
          <div key={m.id} className="flex items-center gap-3 px-3 py-2 rounded-xl hover:bg-gray-50 transition-colors">
            <div className="relative shrink-0">
              ...
              <span className={`absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white ${statusColor(m.status)}`} />
            </div>
            ...
            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${
              m.status === "online" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"
            }`}>
              {m.status === "online" ? "在线" : "离开"}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="px-4 pb-4 pt-2 border-t border-gray-100">
        <p className="text-xs text-gray-400 mb-2">邀请链接（所有模块协作可见）</p>
        <div className="flex items-center gap-2 p-2.5 rounded-xl bg-gray-50 border border-gray-200">
          <span className="flex-1 text-xs text-gray-500 truncate">{inviteLink}</span>
          <button
            onClick={handleCopy}
            className="shrink-0 w-6 h-6 rounded-lg bg-violet-100 hover:bg-violet-200 flex items-center justify-center transition-colors"
          >
            {copied ? <Check className="w-3 h-3 text-violet-600" /> : <Copy className="w-3 h-3 text-violet-500" />}
          </button>
        </div>
        <p className="text-[10px] text-gray-400 mt-2">团队成员可在所有模块查看协作内容</p>
      </div>
    </div>
  );
}
...
// ─── TopNav ───────────────────────────────────────────────────────────────────
export default function TopNav({ onLoginClick }: { onLoginClick?: () => void } = {}) {
  ...
  return (
    ...
    <header className="sticky top-0 z-50 w-full bg-white/90 backdrop-blur-xl border-b border-gray-100">
      <div className="max-w-[1440px] mx-auto px-6 h-14 flex items-center gap-4">
        ...
        {/* Right side: AI search + Team + Avatar */}
        <div className="flex items-center gap-2 shrink-0 ml-auto">
          ...
          {isAuthenticated && (
            <div ref={teamRef} className="relative">
              ...
              >
                ...
                <span className="hidden sm:block">团队</span>
                {/* Online dot */}
          ...
          {isAuthenticated ? (
            ...
            <button
              onClick={openLogin}
              className="px-4 py-1.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold transition-colors"
            >
              登录
            </button>
          )}
    ...
    {/* Login modal (only when TopNav manages its own state) */}
    {!onLoginClick && (
